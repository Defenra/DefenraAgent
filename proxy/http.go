...
func (s *HTTPProxyServer) handleRequest(w http.ResponseWriter, r *http.Request) {
	...
	if !connLimiter.CheckConnection(r.RemoteAddr) {
		log.Printf("[HTTP] Connection blocked at connection level for %s", clientIP)
		...
		if domainConfig.Domain == parentDomain {
			for _, record := range domainConfig.DNSRecords {
				if record.Type == "CNAME" && record.Name == subdomain {
					log.Printf("[HTTP] Resolving CNAME: %s -> %s", host, record.Value)
...
func (s *HTTPProxyServer) proxyRequest(w http.ResponseWriter, r *http.Request, target string) {
	...
	for key, values := range r.Header {
		...
		// This ensures we can inspect and modify HTML content
